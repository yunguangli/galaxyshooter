"""
test_scene.py — Step 5 interactive test for Scene.

Same shoot-em-up as test_collision.py, but all the canvas / input / background
boilerplate is replaced by a single Scene object.

Compare with test_collision.py to see what Scene eliminates:

  BEFORE (test_collision.py)          AFTER (test_scene.py)
  ─────────────────────────────────   ──────────────────────────────────
  canvas = ft.Stack(...)              scene = Scene(page, W, H, "#111122")
  canvas.controls.append(bg_cont)     (automatic)
  inp = InputManager(page)            (automatic)
  wrapped = inp.wrap(canvas)          (automatic)
  page.add(wrapped)                   scene.mount()
  canvas.controls.append(x.control)  scene.add(x)
...
- Bullet hits enemy  → burst SFX + particle burst.
- Player touches enemy → ring SFX + expanding ring + health loss.
- Health 0  → Game Over dialog.
- All enemies gone → You Win dialog.

Run:  cd src && flet run test_scene.py
"""
...
from flet_game import (
    Sprite, GameLoop, Label, Scene,
    Collider, Effects, Audio, make_beep, audio_available,
)
...
ENEMY_SPEED_MAX = 130
MAX_HEALTH     = 5
HURT_COOLDOWN  = 0.8      # s between player-hurt events


def main(page: ft.Page) -> None:
    page.title = "flet_game — Scene (Step 5)"
    ...
    # ── Scene ─────────────────────────────────────────────────────────────────
    # One call replaces: ft.Stack, background Container, InputManager.wrap(),
    # and page.add().

    scene = Scene(page, width=W, height=H, bgcolor="#111122")

    # ── HUD labels (z=10 → drawn on top of all game objects) ─────────────────
    ...
    for lbl in (score_label, health_label, hint_label):
        scene.add(lbl, z=10)

    # ── Visual effects ────────────────────────────────────────────────────────

    fx = Effects(page, scene.canvas)
    ...
    # ── Input — use scene.input (pre-wired InputManager) ─────────────────────
    ...
    def update(dt: float) -> None:
        ...
        for e in enemies:
            if e.visible:
                e.x -= enemy_speeds[id(e)] * dt
                if e.x < -ENEMY_W:
                    e.x = W + 20
                    e.y = max(0, min(H - ENEMY_H,
                              e.y + (50 if e.y < H / 2 else -50)))
